...
SWFUpload.Console.writeLine = function(d) {
    ...
    try {
        ...
        ui.error("Exception: " + c.name + " Message: " + c.message)
    }
};
...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '该文件不能加入上传队列 ';
		...
		onDialogClose :  function(filesSelected, filesQueued, queueLength) {
			...
			if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					ui.error(this.queueData.errorMsg);
				}
			}
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件 "' + file.name + '" 已经出现在队列中。 \n确定想替换掉在队列中已存在的文件?');
					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '"超出上传文件允许最大值 (' + settings.fileSizeLimit + ').';
						break;
		...
		onUploadError : function(file, errorCode, errorMsg) {
			...
			switch(errorCode) {
				...
				case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
					ui.error('The upload limit has been reached (' + errorMsg + ').');
		...
		onUploadStart : function(file) {
			...
			// byron sampson 修改于2014/02/20 通过文件类型选择服务器 start
			var ext = file.type.toLowerCase();
			if(typeof(settings.vodFileExt)!='undefined' && settings.vodFileExt.indexOf('*'+ext+';')!=-1){
				this.settings.upload_url = (typeof(settings.vodUploader) != 'undefined' ? settings.vodUploader : settings.uploader);
				this.settings.post_params = (typeof(settings.vodFormData) != 'undefined' ? settings.vodFormData : settings.formData);
			}else if(typeof(settings.resFileExt)!='undefined' && settings.resFileExt.indexOf('*'+ext+';')!=-1){
				this.settings.upload_url = (typeof(settings.resUploader) != 'undefined' ? settings.resUploader : settings.uploader);
				this.settings.post_params = (typeof(settings.resFormData) != 'undefined' ? settings.resFormData : settings.formData);
			}else{
				this.settings.upload_url = settings.uploader;
				this.settings.post_params = (typeof(settings.FormData) != 'undefined' ? settings.FormData : {});
				// 将文件域表单的名字填入post数据中
				$(this.settings.post_params).attr('file_post_name',settings.fileObjName);
			}
			swfuploadify = window['uploadify_' + settings.id];
			swfuploadify.setUploadURL(this.settings.upload_url);
			swfuploadify.setPostParams(this.settings.post_params);
			// byron sampson 修改于2014/02/20 通过文件类型选择服务器 end
		...
		onUploadSuccess : function(file, data, response) {
            ...
            //rickeryu Add 用于判断不同类型的东西，对应的信息，插入到数据库当中
            var result = {};
            var resultdata;
            var ext = file.type.toLowerCase();
            if(typeof(settings.vodFileExt)!='undefined' && settings.vodFileExt.indexOf('*'+ext+';')!=-1){
                ...
                var obj = eval('('+data+')');
                //{"filedir":"files/2014-04","outfilename":"dlndhm3igcyi88w1.mp4","oldname":"excel_vba2_条件结构","filcode":"8w069toieuxvmmz0"}
                result['fjmc'] = obj['oldname'];
                result['fwq'] = settings.vodUrl;
                result['fjlj'] = obj['filedir']+'/';
                result['fjlx'] = 'application/octet-st';
                result['fwdmc'] = obj['outfilename'];
                result['fcode'] = obj['filcode'];
                result['ext'] = 'mp4';
                ...
                var obj = eval('('+data+')');
                if(obj.status == 1){
                    //{"name":"高教智慧课堂可进行api.xlsx","type":"application/octet-stream","size":"9052","ext":".xlsx","savename":"97a7100ea19e5865.xlsx","filecode":"97a7100ea19e5865","webpath":"resource/2014/4/21/10/4/","convertAble":"1","result":"true","info":"上传成功","status":"1"}
                    result['fjmc'] = obj['name'];
                    result['fwq'] = settings.resUrl;
                    result['fjlj'] = obj['webpath'];
                    result['fjlx'] = obj['type'];
                    result['fwdmc'] = obj['savename'];
                    result['size'] = obj['size'];
                    result['fcode'] = obj['filecode'];
                    result['ext'] = ext.replace('.','');
                }
                //console.log(file);console.log(data);
            }else{
                //其他的正常操作
            ...
            if(typeof(result['fjmc']) != 'undefined' && result['fjmc'] != ''){
